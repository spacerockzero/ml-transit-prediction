#!/usr/bin/env python3
"""
Enhanced synthetic shipping data generator for transit time and cost prediction.
Generates realistic shipping datasets with proper zone-based calculations.

Updated Sept 2025 with real-world analytics data patterns:
- Base transit times calibrated to actual Zone 1-9 medians
- Carrier effects based on observed performance rankings
- Cost parameters updated from 50K shipment analysis
- Maintains proper statistical distributions and variability
"""
...
# Updated based on real-world analytics data analysis (Sept 2025)
base_transit_times = {
    "Overnight": {
        1: 1.3,
        2: 1.4,
    ...
    "Express": {1: 2.2, 2: 2.3, 3: 2.5, 4: 2.7, 5: 2.8, 6: 3.0, 7: 3.1, 8: 3.3, 9: 3.5},
    "Priority": {
        1: 3.4,
        2: 3.6,
        3: 3.7,
    ...
    "Ground": {1: 5.7, 2: 6.0, 3: 6.4, 4: 6.6, 5: 7.0, 6: 7.3, 7: 7.6, 8: 8.1, 9: 8.1},
}

# Define base shipping costs (in USD) based on real carrier pricing data
# Updated with actual cost analysis from 50K shipment dataset
base_shipping_costs = {
    "Overnight": {
        1: 28.0,
        2: 28.5,
...
def calculate_realistic_transit_time_and_cost(
    origin_zip, dest_zip, service_level, ship_date, package_weight, package_dimensions
):
    ...
    # Carrier efficiency (updated based on 50K shipment analytics)
    # Values represent days faster(-) or slower(+) than base time
    carrier_effect_time = {
        "USPS": 0.0,  # Baseline reference
        "FedEx": -0.4,  # Consistently fastest
        "UPS": -0.2,  # Second fastest
        "DHL": 0.2,  # Slightly slower
        "Amazon_Logistics": 0.1,  # Near baseline
        "OnTrac": 0.3,  # Regional limitations
        "LaserShip": 0.5,  # Slowest but budget
        "Regional_Express": 0.6,
    }

    # Carrier pricing (based on real market analysis)
    # Multipliers applied to base cost
    carrier_effect_cost = {
        "USPS": 0.82,  # Government pricing advantage
        "FedEx": 1.15,  # Premium positioning
        "UPS": 1.08,  # Slight premium over USPS
        "DHL": 1.25,  # Most expensive
        "Amazon_Logistics": 0.88,  # Competitive tech pricing
        "OnTrac": 0.92,  # Regional competitive
        "LaserShip": 0.78,  # Budget option
        "Regional_Express": 0.88,
    }
    ...
    dim_weight = (
        package_dimensions[0] * package_dimensions[1] * package_dimensions[2]
    ) / 166
    ...
    shipping_cost = (
        base_cost
        * carrier_effect_cost.get(carrier, 1.0)
        * weight_cost_factor
        * random_factor_cost
    )
...
for date in date_range:
    ...
    for _ in range(daily_shipments):
        ...
        transit_time, shipping_cost, carrier = (
            calculate_realistic_transit_time_and_cost(
                origin_zip,
                dest_zip,
                service_level,
                date,
...
cost_missing_indices = np.random.choice(
...
df.loc[outlier_indices, "transit_time_days"] = df.loc[
    outlier_indices, "transit_time_days"
] + np.random.uniform(10, 25, len(outlier_indices))
...
df.drop(["shipment_id", "route", "zone_route"], axis=1).to_parquet(
...
df.drop(["shipment_id", "route", "zone_route"], axis=1).to_csv(
    "output/historical_shipments.csv", index=False
)
