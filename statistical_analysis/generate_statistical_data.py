...
def generate_statistical_dataset(n_records=50000):
    ...
    for i in range(n_records):
        ...
        shipping_cost = generate_shipping_cost(service_level, dest_zone, carrier, weight, volume)
...
def generate_distribution_metadata(df):
    ...
    metadata = {
        ...
        'date_range': {
            'start': df['ship_date'].min(),
            'end': df['ship_date'].max()
        },
...
def main():
    ...
    for service in metadata['service_levels']:
        service_data = df[df['service_level'] == service]
        print(f"  {service}: {service_data['transit_time_days'].mean():.2f} ± {service_data['transit_time_days'].std():.2f} days")
    
    print("\nSHIPPING COST STATISTICS BY SERVICE LEVEL:")
    for service in metadata['service_levels']:
        service_data = df[df['service_level'] == service]
        print(f"  {service}: ${service_data['shipping_cost_usd'].mean():.2f} ± ${service_data['shipping_cost_usd'].std():.2f}")
